...
public class UserServiceConsumerTest_13_AddMinArrayLikeToThourthDto {
    ...
    public void verifyCreatePersonPact() {
        ...
        IdObject id = userClient.createCar(new RequestWrapperDto<>(new Car(user, user,user, List.of(user))), headers);
    ...
    public void verifyCreatePersonPactReactiveFeign() {
        ...
        user.setFirstName("Zaphod");
        user.setLastName("Beeblebrox");
        StepVerifier.create(reactiveUserClient.createCar(new RequestWrapperDto<>(new Car(user, user,user,List.of(user))), headers))
                .assertNext(id -> {
                    assertEquals(id.getId(), 42);
                }).verifyComplete();
    }
    ...
    public void verifyCreatePersonPactWebClient() {
        ...
        var idObjectMono = webClient
                ...
                .body(BodyInserters.fromValue(new RequestWrapperDto(new Car(user, user,user,List.of(user)))))
                .retrieve()
